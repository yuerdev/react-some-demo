...
const isAccessControlComponentOrHook = (id: string) => isAccessControlEveryComponentOrHook(id) || isAccessControlSomeComponentOrHook(id);
...
export default () => ({
  name: 'label-plugin-access-control',
  visitor: {
      ...
      programNodePath.traverse({
        VariableDeclarator: {
          enter(variableDeclPath: NodePath<VariableDeclarator>) {
            if (isIdentifier(variableDeclPath.node.id) && isAccessControlComponentOrHook(variableDeclPath.node.id.name)) {
